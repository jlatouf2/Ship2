...
/*********************(****** 2D BOX FCNS ********)**************************/
  ...
  //		ctx.translate(-carGame.camera.x, -carGame.camera.y);

		ctx.clearRect(0, 0, canvasWidth, canvasHeight);


		// Display the build-in debug drawing.
		if (shouldDrawDebug) { carGame.world.DrawDebugData();  }
		...
		carGame.world.ClearForces();
		 	...
		 	/*	if (objectMovement == true) {
				//	console.log(carGame.world.GetBodyList());
					var body = carGame.world.GetBodyList();
					ctx.save();
					ctx.translate(carGame.mega.x - canvasWidth / 2, carGame.mega.y - canvasHeight / 2);
...
*/
	...
	function drawWorld(world, context) {
		for (var body = carGame.world.GetBodyList(); body != null;
			...
			if (body.GetUserData() !== null && body.GetUserData() !== undefined) {
				...
				context.save();
				 ...
				 camera.x = Math.floor(carGame.mega.x + (carGame.mega.width / 2) - (canvasWidth / 2));
				 camera.y = Math.floor(carGame.mega.y + (carGame.mega.height / 2) - (canvasHeight / 2));
		...
		//	 	context.translate(x * pxPerMeter,y * pxPerMeter);
		  	 ...
		  	 //  context.translate(carGame.mega.x - canvasWidth / 2, carGame.mega.y - canvasHeight / 2);
